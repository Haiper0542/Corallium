...
+ **조작법**
   + 좌측 조이스틱 : 플레이어 이동
* **개발시 난관**
   * VR과 NonVR을 오가는 것은 기본으로 제공되는 sdk로 구현하기는 어려웠음
      * 핸드폰의 자이로 센서값을 받아와서 직접 스크립트로 화면전환을 함
      * 또한 화면을 담당하는 카메라를 2개로 나누어서 해결함
   * 수족관 속 물고기의 움직임은 그저 랜덤 좌표로는 구현하기 어려웠음
      * Flocking AI 알고리즘을 이용하여 물고기가 때로 다닐 수 있도록 인공지능을 구현함
   * 해초류의 모델링을 담당하던 모델러가 없어져서 해초류의 모델이 없었음
      * 마인크래프트의 잔디처럼 sprite2개를 X자로 엇갈려놓음으로써 어느정도 자연스러운 해초를 구현함
### 게임 화면
